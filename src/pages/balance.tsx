...
import { Coins, Send, Clock, Wallet, TrendingUp, Sparkles } from "lucide-react";
  ...
  return (
    <Layout>
      <div className="space-y-8 max-w-4xl mx-auto">
        ...
        {/* Main Balance Card */}
        <div className="premium-card-glow rounded-2xl overflow-hidden relative">
          ...
          <div className="relative p-8">
            <div className="flex items-start justify-between mb-6">
              <div>
                <p className="text-xs text-zinc-500 uppercase tracking-wider font-bold mb-1">Available Balance</p>
            ...
            {isLoading ? (
              <Skeleton className="h-16 w-48 bg-zinc-800 rounded-xl" />
            ) : (
              <div className="flex items-baseline gap-3">
                <span className="text-6xl font-black gold-gradient-text">${formatMoney(balanceData?.balance)}</span>
                <span className="text-2xl font-bold text-amber-400">USDT</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="premium-card rounded-xl p-6">
            ...
            <p className="text-xs text-zinc-500 uppercase tracking-wider font-bold mb-2">Total Earned</p>
            {isLoading ? (
              <Skeleton className="h-8 w-28 bg-zinc-800 rounded" />
            ) : (
              <span className="text-2xl font-black text-white">{formatMoney(balanceData?.totalEarned)} USDT</span>
            )}
          </div>
          ...
          <div className="premium-card rounded-xl p-6">
            ...
            <p className="text-xs text-zinc-500 uppercase tracking-wider font-bold mb-2">Pending Withdrawals</p>
            {isLoading ? (
              <Skeleton className="h-8 w-28 bg-zinc-800 rounded" />
            ) : (
              <span className="text-2xl font-black text-white">{formatMoney(balanceData?.pendingWithdrawals)} USDT</span>
            )}
          </div>
        </div>
      </div>
